...
1. **Spec compliance** — does the code satisfy every acceptance criterion in `requirements.md` and
   ...
   over-building.
2. **Code quality** — only after pass 1: does it obey `AGENTS.md` (the single coding checklist —
   constructor injection, no blocking I/O / DB / sync calls in the topology, explicit serdes, error
   handling, layer boundaries, tests)?
...
(`review_type: code`, the round number, the verdict). Do not edit any source file. Return a one-line
verdict: `approved` or `changes-requested (N blocker, M major)`.
